...
const LumiSmall = () => <Lumi size={28} />;

// ─── DESIGN TOKENS (Warm, Light, Organic) ───
const C = {
  bg: "#FFFCF5",
  bgWarm: "#FFF8EB",
  bgCard: "#FFFFFF",
...
const css = `
  ...
  *{margin:0;padding:0;box-sizing:border-box;-webkit-tap-highlight-color:transparent}
  ...
  @keyframes lumiBounce{0%,100%{transform:scale(1)}50%{transform:scale(1.1)}}
...
// ─── COMPONENTS ───
const Scr = ({children,bg})=><div style={{height:"100vh",overflowY:"auto",paddingTop:20,paddingBottom:90,paddingLeft:20,paddingRight:20,background:bg||C.bg,WebkitOverflowScrolling:"touch"}}>{children}</div>;
...
const Btn = ({children,onClick,v="primary",disabled,full=true,style:sx={}})=>{
  ...
  return<button disabled={disabled} onClick={onClick} style={{padding:"13px 24px",borderRadius:14,fontSize:15,fontWeight:700,fontFamily:C.font,cursor:disabled?"default":"pointer",opacity:disabled?.5:1,width:full?"100%":"auto",...st[v],...sx}}>{children}</button>
};

const Card = ({children,bg,onClick,style:sx={}})=><div onClick={onClick} style={{background:bg||C.bgCard,borderRadius:20,padding:18,border:`1px solid ${C.border}`,boxShadow:"0 2px 8px rgba(45,36,23,0.04)",cursor:onClick?"pointer":"default",transition:"all .2s",...sx}}>{children}</div>;

const Pill = ({text,color,bg})=><span style={{fontSize:10,padding:"3px 10px",borderRadius:20,background:bg||`${color}18`,color:color,fontFamily:C.font,fontWeight:700,textTransform:"uppercase",letterSpacing:.5}}>{text}</span>;

const Dots = ()=>{const[f,sF]=useState(0);useEffect(()=>{const i=setInterval(()=>sF(n=>(n+1)%4),400);return()=>clearInterval(i)},[]);return<div style={{display:"flex",gap:5,padding:"10px 0"}}>{[0,1,2].map(i=><div key={i} style={{width:7,height:7,borderRadius:"50%",background:C.lumi,opacity:f>i?.85:.2,transition:"opacity .3s"}}/>)}</div>};

const Bub = ({from,text,typing})=><div className="fu" style={{display:"flex",justifyContent:from==="user"?"flex-end":"flex-start",marginBottom:12}}>
...
const AuthScreen = () => {
  ...
  if(mode==="check")return<div style={{minHeight:"100vh",background:`linear-gradient(180deg,${C.bgWarm} 0%,${C.bg} 100%)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:40}}>
    <Lumi size={64} mood="excited" animate/><div style={{height:20}}/>
    ...
    <p style={{color:C.textBody,fontSize:14,fontFamily:C.font,textAlign:"center",lineHeight:1.7,maxWidth:300}}>We sent a confirmation link to <strong>{email}</strong>. Click it, then come back and sign in.</p>
    <div style={{marginTop:24,width:"100%",maxWidth:280}}><Btn v="ghost" onClick={()=>setMode("signin")}>← Back to Sign In</Btn></div>
  </div>;

  return<div style={{minHeight:"100vh",background:`linear-gradient(180deg,${C.bgWarm} 0%,${C.bg} 100%)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 28px"}}>
    ...
    <div style={{width:"100%",maxWidth:340}}>
      <div className="fu s3" style={{display:"flex",gap:6,marginBottom:20,background:C.bgCard,borderRadius:12,padding:4,border:`1px solid ${C.border}`}}>
        {["signin","signup"].map(m=><button key={m} onClick={()=>{setMode(m);setErr("")}} style={{flex:1,padding:"10px 0",borderRadius:10,border:"none",background:mode===m?C.accentLight:"transparent",color:mode===m?C.accentDark:C.textMuted,fontSize:14,fontWeight:700,fontFamily:C.font,cursor:"pointer"}}>{m==="signin"?"Sign In":"Sign Up"}</button>)}
      </div>
...
const Onboarding = ({uid,onDone})=>{
  ...
  const steps=[
    ...
    {q:"How much do you know about AI?",sub:"No wrong answer here — we all start somewhere!",opts:["Brand new — what even is AI?","I've played with ChatGPT","I use AI tools sometimes","I'm ready to go deeper"],key:"exp"},
  ...
  const pick=(val)=>{
    const s=steps[step];
    if(s.multi){const c=ans[s.key]||[];setAns({...ans,[s.key]:c.includes(val)?c.filter(v=>v!==val):[...c,val]})}
    else{const na={...ans,[s.key]:val};setAns(na);if(step<2)setTimeout(()=>setStep(step+1),300);else finish(na)}
  };
  ...
  if(building)return<div style={{height:"100vh",background:`linear-gradient(180deg,${C.bgWarm},${C.bg})`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:40}}>
    <Lumi size={72} mood="thinking" animate/><div style={{height:16}}/>
  ...
  const s=steps[step];
  return<div style={{minHeight:"100vh",background:`linear-gradient(180deg,${C.bgWarm},${C.bg})`,display:"flex",flexDirection:"column",padding:"48px 24px 40px"}}>
    <div style={{display:"flex",gap:8,marginBottom:32,justifyContent:"center"}}>{steps.map((_,i)=><div key={i} style={{width:i===step?32:10,height:10,borderRadius:5,background:i<=step?C.accent:`${C.text}12`,transition:"all .4s"}}/>)}</div>
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}><Lumi size={36} mood={step===2?"excited":"happy"}/><span style={{color:C.textMuted,fontSize:12,fontFamily:C.font,fontWeight:600}}>Lumi wants to know...</span></div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:10,flex:1}}>
      {s.opts.map((o,i)=>{const sel=s.multi?(ans[s.key]||[]).includes(o):ans[s.key]===o;return<button key={o} className={`pop s${Math.min(i+1,5)}`} onClick={()=>pick(o)} style={{background:sel?C.accentLight:C.bgCard,border:`2px solid ${sel?C.accent:C.border}`,borderRadius:16,padding:"14px 18px",cursor:"pointer",textAlign:"left",boxShadow:sel?"0 2px 12px rgba(232,149,46,0.15)":"0 1px 4px rgba(0,0,0,0.03)"}}>
        <span style={{color:sel?C.accentDark:C.textBody,fontSize:15,fontWeight:sel?700:500,fontFamily:C.font}}>{sel?"✓  ":""}{o}</span>
      </button>})}
    </div>
    {s.multi&&(ans[s.key]||[]).length>0&&<div className="fu" style={{marginTop:16}}><Btn onClick={()=>finish()}>Let's go! →</Btn></div>}
  </div>;
};

// ─── TAB BAR ───
const TabBar = ({tab,setTab})=><div style={{position:"fixed",bottom:0,left:0,right:0,height:72,background:`${C.bg}f5`,backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",borderTop:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-around",zIndex:100,paddingBottom:"env(safe-area-inset-bottom,0)"}}>
  {[{id:"learn",icon:"📚",l:"Learn"},{id:"news",icon:"📰",l:"News"},{id:"tools",icon:"🧰",l:"Tools"},{id:"profile",icon:"😊",l:"Me"}].map(t=>{const a=tab===t.id;return<button key={t.id} onClick={()=>setTab(t.id)} style={{background:a?C.accentLight:"none",border:"none",display:"flex",flexDirection:"column",alignItems:"center",gap:2,cursor:"pointer",padding:"8px 20px",borderRadius:14,transition:"all .2s"}}>
...
const LearnTab = ({profile,progress,onOpen})=>{
  ...
  return<Scr bg={`linear-gradient(180deg,${C.bgWarm} 0%,${C.bg} 30%)`}>
    {/* Header */}
    ...
    {/* Lumi tip */}
    {progress.length===0&&<Card bg={C.bgWarm} style={{marginBottom:20,display:"flex",alignItems:"center",gap:14,border:`1.5px solid ${C.lumiLight}`}}>
      <Lumi size={40} mood="happy"/>
      <div><p style={{color:C.text,fontSize:14,fontWeight:700,fontFamily:C.font,margin:"0 0 2px"}}>Welcome! Start with AI Basics</p><p style={{color:C.textMuted,fontSize:12,fontFamily:C.font,margin:0,lineHeight:1.5}}>I picked this path for you — it takes just 5 minutes to complete your first lesson!</p></div>
    </Card>}
    ...
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      {PATHS.map((p,i)=>{
        ...
        return<Card key={p.id} onClick={()=>onOpen(p.id)} style={{padding:0,overflow:"hidden"}} bg={C.bgCard}>
          <div className={`fu s${Math.min(i+1,5)}`} style={{display:"flex",alignItems:"center",gap:16,padding:16}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <div style={{flex:1,height:6,background:"#F0EBE0",borderRadius:6,overflow:"hidden"}}><div style={{width:`${pct}%`,height:"100%",background:`linear-gradient(90deg,${p.color},${p.color}cc)`,borderRadius:6,transition:"width .6s ease"}}/></div>
...
const LessonView = ({pathId,uid,onBack,onComplete})=>{
  const lesson=LESSONS[pathId]||LESSONS.basics;const path=PATHS.find(p=>p.id===pathId)||PATHS[0];
  const [showTutor,setShowTutor]=useState(false);const [msgs,setMsgs]=useState([]);const [typing,setTyping]=useState(false);const [inp,setInp]=useState("");const [sid,setSid]=useState(null);const ref=useRef(null);
  ...
  const ask=async(q)=>{setMsgs(m=>[...m,{from:"user",text:q}]);setTyping(true);
    try{const r=await db.callClaude({feature:"tutor",system:`You are Lumi, a warm and friendly AI tutor inside "AI Fluent" teaching "${lesson.title}". The user is a complete beginner. Use simple everyday language, fun analogies, and occasional emojis. Keep responses under 180 words. Be encouraging — celebrate when they ask good questions!`,messages:[...msgs.map(m=>({role:m.from==="user"?"user":"assistant",content:m.text})),{role:"user",content:q}],session_id:sid,context_type:"tutor",context_id:pathId,context_title:lesson.title});
      setSid(r.session_id||sid);setTyping(false);setMsgs(m=>[...m,{from:"lumi",text:r.text}]);
    }catch{setTyping(false);setMsgs(m=>[...m,{from:"lumi",text:"Hmm, I'm having a little trouble connecting right now. Give me a moment and try again! 😊"}])}};
  const send=()=>{if(inp.trim()){ask(inp.trim());setInp("")}};

  if(showTutor)return<div style={{height:"100vh",display:"flex",flexDirection:"column",background:C.bg}}>
    <div style={{padding:"14px 16px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:10,flexShrink:0,background:C.bgWarm}}>
      ...
      <div style={{flex:1,textAlign:"center",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}><Lumi size={24}/><span style={{color:C.text,fontSize:14,fontWeight:700,fontFamily:C.font}}>Lumi — AI Tutor</span></div>
    </div>
    <div ref={ref} style={{flex:1,overflowY:"auto",padding:16,background:`linear-gradient(180deg,${C.bgWarm},${C.bg})`}}>
      <Bub from="lumi" text={`Hey there! 👋 I'm Lumi, your AI tutor for "${lesson.title}"\n\nAsk me anything about what you just read — no question is too simple! I love explaining things.`}/>
      {msgs.map((m,i)=><Bub key={i} from={m.from} text={m.text}/>)}{typing&&<Bub from="lumi" typing/>}
      {msgs.length===0&&<div className="fu s2" style={{marginTop:16}}>
        <p style={{color:C.textMuted,fontSize:12,fontFamily:C.font,fontWeight:600,margin:"0 0 10px"}}>💡 Try asking...</p>
        {lesson.questions.map((q,i)=><button key={i} onClick={()=>ask(q)} style={{display:"block",width:"100%",background:C.bgCard,border:`1.5px solid ${C.border}`,borderRadius:14,padding:"11px 16px",marginBottom:8,cursor:"pointer",textAlign:"left",boxShadow:"0 1px 4px rgba(0,0,0,0.03)"}}>
          <span style={{color:C.textBody,fontSize:14,fontFamily:C.font}}>{q}</span></button>)}</div>}
    </div>
    <div style={{padding:"10px 16px 14px",borderTop:`1px solid ${C.border}`,display:"flex",gap:8,flexShrink:0,background:C.bgCard,paddingBottom:"max(14px,env(safe-area-inset-bottom))"}}>
      ...
      <button onClick={send} style={{width:44,height:44,borderRadius:14,background:`linear-gradient(135deg,${C.accent},${C.accentDark})`,border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,boxShadow:"0 3px 10px rgba(232,149,46,0.3)"}}>
        <span style={{color:"#fff",fontSize:18}}>↑</span></button>
    </div></div>;

  return<Scr bg={`linear-gradient(180deg,${path.bg},${C.bg} 20%)`}>
    ...
    <div className="fu" style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}><Pill text="Lesson 1" color={path.color} bg={path.bg}/></div>
    ...
    <div className="fu s2" style={{display:"flex",alignItems:"center",gap:8,marginBottom:28}}><Lumi size={20}/><span style={{color:C.textMuted,fontSize:12,fontFamily:C.font}}>Powered by Claude</span></div>

    {lesson.sections.map((sec,i)=><div key={i} className={`fu s${Math.min(i+2,5)}`} style={{marginBottom:28}}>
    ...
    <Card bg={C.bgWarm} onClick={()=>setShowTutor(true)} style={{marginBottom:16,display:"flex",alignItems:"center",gap:14,border:`1.5px solid ${C.lumiLight}`,cursor:"pointer"}}>
      ...
      <div><p style={{color:C.text,fontSize:16,fontWeight:700,fontFamily:C.font,margin:"0 0 3px"}}>Ask Lumi about this lesson</p><p style={{color:C.textMuted,fontSize:13,fontFamily:C.font,margin:0}}>Your friendly AI tutor is ready to help!</p></div>
    </Card>
...
const NewsTab = ({uid})=>{
  ...
  const ask=async(q)=>{setMsgs(m=>[...m,{from:"user",text:q}]);setTyping(true);
    try{const r=await db.callClaude({feature:"news",system:`You are Lumi, a warm AI news explainer. The user is reading: "${art.title}". Summary: ${art.eli5}. Explain in everyday language. Keep under 150 words. Use an emoji occasionally.`,messages:[...msgs.map(m=>({role:m.from==="user"?"user":"assistant",content:m.text})),{role:"user",content:q}],session_id:sid,context_type:"news",context_id:String(art.id),context_title:art.title});
      setSid(r.session_id||sid);setTyping(false);setMsgs(m=>[...m,{from:"lumi",text:r.text}]);
    }catch{setTyping(false);setMsgs(m=>[...m,{from:"lumi",text:"Oops — having trouble connecting. Try again in a moment! 😊"}])}};
  const send=()=>{if(inp.trim()){ask(inp.trim());setInp("")}};

  if(chat&&art)return<div style={{height:"100vh",display:"flex",flexDirection:"column",background:C.bg}}>
    <div style={{padding:"14px 16px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:10,flexShrink:0,background:C.bgWarm}}>
      ...
      <div style={{flex:1,textAlign:"center",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}><Lumi size={24}/><span style={{color:C.text,fontSize:14,fontWeight:700,fontFamily:C.font}}>Ask Lumi</span></div>
    </div>
    <div ref={ref} style={{flex:1,overflowY:"auto",padding:16,background:`linear-gradient(180deg,${C.bgWarm},${C.bg})`}}>
      <Bub from="lumi" text={`I read "${art.title}" for you! 📰\n\nAsk me anything — I'll break it down in plain language.`}/>
      {msgs.map((m,i)=><Bub key={i} from={m.from} text={m.text}/>)}{typing&&<Bub from="lumi" typing/>}
      {msgs.length===0&&<div className="fu" style={{marginTop:16}}><p style={{color:C.textMuted,fontSize:12,fontFamily:C.font,fontWeight:600,margin:"0 0 10px"}}>💡 People are asking...</p>
        {art.questions.map((q,i)=><button key={i} onClick={()=>ask(q)} style={{display:"block",width:"100%",background:C.bgCard,border:`1.5px solid ${C.border}`,borderRadius:14,padding:"11px 16px",marginBottom:8,cursor:"pointer",textAlign:"left"}}><span style={{color:C.textBody,fontSize:14,fontFamily:C.font}}>{q}</span></button>)}</div>}
    </div>
    <div style={{padding:"10px 16px 14px",borderTop:`1px solid ${C.border}`,display:"flex",gap:8,flexShrink:0,background:C.bgCard}}>
      <input value={inp} onChange={e=>setInp(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="Ask about this story..."
        style={{flex:1,background:C.bg,borderRadius:14,border:`1.5px solid ${C.border}`,padding:"11px 16px",color:C.text,fontSize:14,fontFamily:C.font,outline:"none"}}/>
      <button onClick={send} style={{width:44,height:44,borderRadius:14,background:`linear-gradient(135deg,${C.accent},${C.accentDark})`,border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><span style={{color:"#fff",fontSize:18}}>↑</span></button></div></div>;
  ...
  if(art)return<Scr>
    ...
    <Card bg={C.greenLight} className="fu s2" style={{marginBottom:14,border:`1px solid ${C.greenBg}`}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}><span style={{fontSize:20}}>💡</span><p style={{color:C.green,fontSize:14,fontWeight:700,fontFamily:C.font,margin:0}}>Explain Like I'm 5</p></div>
    ...
    <Card bg={C.bgPeach} className="fu s3" style={{marginBottom:14,border:`1px solid #F5E0D0`}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}><span style={{fontSize:20}}>🎯</span><p style={{color:C.coral,fontSize:14,fontWeight:700,fontFamily:C.font,margin:0}}>Why It Matters To You</p></div>
    ...
    <Card bg={C.bgWarm} onClick={()=>setChat(true)} className="fu s4" style={{display:"flex",alignItems:"center",gap:14,border:`1.5px solid ${C.lumiLight}`,cursor:"pointer"}}>
      ...
      <div><p style={{color:C.text,fontSize:15,fontWeight:700,fontFamily:C.font,margin:"0 0 3px"}}>Still have questions?</p><p style={{color:C.textMuted,fontSize:13,fontFamily:C.font,margin:0}}>Ask Lumi to explain anything!</p></div>
    </Card>
  </Scr>;

  return<Scr><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
    <div><p style={{color:C.textMuted,fontSize:13,fontFamily:C.font,fontWeight:600}}>Daily Digest 📬</p><h1 style={{color:C.text,fontSize:26,fontFamily:C.fontDisplay,fontWeight:700}}>AI News</h1></div>
    <Lumi size={36} mood="happy"/></div>
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      {NEWS.map((n,i)=><Card key={n.id} onClick={()=>setOpen(n.id)} bg={n.color} style={{border:"none",padding:0,overflow:"hidden"}} className={`fu s${Math.min(i+1,5)}`}>
        <div style={{padding:"18px 18px 14px",position:"relative"}}>
          <div style={{position:"absolute",top:14,right:16,fontSize:28,opacity:.5}}>{n.emoji}</div>
        ...
        <div style={{background:`${C.bgCard}99`,padding:"10px 18px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <div style={{display:"flex",gap:6}}><Pill text="💡 ELI5" color={C.green}/><Pill text="✦ Ask Lumi" color={C.accent}/></div>
        </div>
      </Card>)}
    </div>
  </Scr>};
...
const ToolsTab = ({uid})=>{
  ...
  const doGen=async(all)=>{setGen(true);try{const prompt=wf.steps.map((s,i)=>`${s.q}: ${all[i]}`).join("\n");const r=await db.callClaude({feature:"tool",system:wf.sys+" You are Lumi from AI Fluent. Keep output practical and beginner-friendly.",messages:[{role:"user",content:prompt}],context_type:"tool",context_id:wf.id,context_title:wf.name});setResult(r.text)}catch{setResult("Hmm, having trouble connecting. Try again in a moment!")}setGen(false)};
  const pick=(v)=>{const na=[...ans,v];setAns(na);if(step<wf.steps.length-1)setTimeout(()=>setStep(step+1),200);else doGen(na)};

  if(result)return<Scr><button onClick={reset} style={{background:"none",border:"none",color:C.accent,fontSize:14,cursor:"pointer",fontFamily:C.font,fontWeight:700,marginBottom:16}}>← Tools</button>
    <div className="fu" style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}><span style={{fontSize:24}}>✨</span><h2 style={{color:C.text,fontSize:20,fontWeight:700,fontFamily:C.fontDisplay,margin:0}}>Here's your result!</h2></div>
    <Card bg={C.bgWarm} className="fu s1" style={{marginBottom:16,border:`1.5px solid ${C.lumiLight}`}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}><Lumi size={24}/><span style={{color:C.textMuted,fontSize:12,fontFamily:C.font,fontWeight:600}}>Created by Lumi</span></div>
  ...
  if(gen)return<div style={{height:"100vh",background:`linear-gradient(180deg,${C.bgWarm},${C.bg})`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:40}}>
    <Lumi size={64} mood="thinking" animate/><div style={{height:16}}/>
    <p className="fu" style={{color:C.text,fontSize:17,fontFamily:C.fontDisplay,fontWeight:700}}>Lumi is working on it...</p>
  </div>;

  if(wf){const s=wf.steps[step];return<Scr>
    ...
    <div style={{display:"flex",gap:4,marginBottom:20}}>{wf.steps.map((_,i)=><div key={i} style={{flex:1,height:5,borderRadius:3,background:i<=step?C.accent:`${C.text}0c`,transition:"all .3s"}}/>)}</div>
    ...
    {s.free?<div className="fu s1"><textarea value={ft} onChange={e=>setFt(e.target.value)} placeholder={s.ph} style={{width:"100%",minHeight:110,background:C.bgCard,borderRadius:16,border:`1.5px solid ${C.border}`,padding:16,color:C.text,fontSize:15,fontFamily:C.font,outline:"none",resize:"vertical",lineHeight:1.6}}/><div style={{marginTop:12}}><Btn onClick={()=>pick(ft||s.ph.replace("e.g. ",""))} disabled={!ft.trim()}>{step<wf.steps.length-1?"Next →":"Create with Lumi ✨"}</Btn></div></div>
    :<div style={{display:"flex",flexDirection:"column",gap:10}}>{s.opts.map((o,i)=><button key={o} className={`pop s${Math.min(i+1,5)}`} onClick={()=>pick(o)} style={{background:C.bgCard,border:`1.5px solid ${C.border}`,borderRadius:16,padding:"14px 18px",cursor:"pointer",textAlign:"left",boxShadow:"0 1px 4px rgba(0,0,0,0.03)"}}><span style={{color:C.textBody,fontSize:15,fontFamily:C.font,fontWeight:500}}>{o}</span></button>)}</div>}
  </Scr>}

  return<Scr><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}>
    <div><p style={{color:C.textMuted,fontSize:13,fontFamily:C.font,fontWeight:600}}>Guided by Lumi</p><h1 style={{color:C.text,fontSize:26,fontFamily:C.fontDisplay,fontWeight:700}}>AI Tools 🧰</h1></div>
    <Lumi size={36} mood="excited"/>
  </div>
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      {WORKFLOWS.map((w,i)=><Card key={w.id} onClick={()=>{setWf(w);setStep(0);setAns([]);setFt("");setResult(null)}} className={`fu s${Math.min(i+1,5)}`} style={{display:"flex",alignItems:"center",gap:14,cursor:"pointer"}}>
        ...
        <div style={{flex:1}}><h3 style={{color:C.text,fontSize:15,fontWeight:700,fontFamily:C.font,margin:0}}>{w.name}</h3><p style={{color:C.textMuted,fontSize:12,fontFamily:C.font,margin:"2px 0 0"}}>{w.desc}</p></div>
  ...
  return<Scr bg={`linear-gradient(180deg,${C.bgWarm} 0%,${C.bg} 30%)`}>
    ...
    <Card bg={`linear-gradient(135deg,${C.accentLight},${C.bgWarm})`} className="fu s1" style={{marginBottom:20,border:`1.5px solid ${C.lumiLight}`}}>
      <div style={{display:"flex",justifyContent:"space-around"}}>
        {[{v:profile?.current_streak||0,l:"Streak",e:"🔥"},{v:progress.length,l:"Lessons",e:"📖"},{v:profile?.total_tutor_sessions||0,l:"Chats",e:"💬"}].map((s,i)=><div key={i} style={{textAlign:"center"}}><span style={{fontSize:22}}>{s.e}</span><p style={{color:C.text,fontSize:22,fontWeight:800,fontFamily:C.font,margin:"2px 0 0"}}>{s.v}</p><p style={{color:C.textMuted,fontSize:11,fontFamily:C.font}}>{s.l}</p></div>)}
      </div>
    </Card>
    ...
    <div className="fu s3" style={{textAlign:"center",marginTop:20}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:6}}><Lumi size={18}/><span style={{color:C.textLight,fontSize:11,fontFamily:C.font}}>AI Fluent v1.0 · Powered by Claude</span></div>
    </div>
  </Scr>};
...
export default function AIFluent(){
    ...
    const init=async()=>{
      try {
        ...
        if(s?.user){
          ...
          setProfile(p);
          const pr=await db.getProgress(s.user.id);
          setProgress(pr);
        }
      } catch(e) {
  ...
  if(loading)return<><style>{css}</style><div style={{height:"100vh",background:`linear-gradient(180deg,${C.bgWarm},${C.bg})`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}><Lumi size={56} mood="happy" animate/><p style={{color:C.textMuted,fontSize:14,fontFamily:"'Nunito',sans-serif",marginTop:16}}>Loading AI Fluent...</p></div></>;
  ...
  if(lesson)return<><style>{css}</style><LessonView pathId={lesson} uid={user.id} onBack={()=>setLesson(null)} onComplete={refresh}/></>;

  return<><style>{css}</style><div style={{background:C.bg,minHeight:"100vh"}}>
    ...
    {tab==="profile"&&<ProfileTab profile={profile} progress={progress} onSignOut={out}/>}
