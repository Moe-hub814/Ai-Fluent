...
import { analytics, analyticsOff, setAnalyticsOff } from "./lib/analytics";
...
const WorldMap = ({user,profile,progress,onOpenLoc,onOpenNews,onOpenTools,onOpenProfile,onOpenChallenge,onOpenChat,onOpenAchievements,onToggleTheme,onChangeLang,onSignIn}) => {
    ...
    <div style={{position:"absolute",top:0,left:0,right:0,padding:`${TOP_SAFE+10}px 12px 10px`,display:"flex",justifyContent:"space-between",alignItems:"center",zIndex:20,
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,minWidth:0,flex:1}}>
        ...
        <div style={{minWidth:0}}>
          <p style={{color:C.text,fontSize:13,fontFamily:C.fontDisplay,fontWeight:700,margin:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{(typeof window!=="undefined"&&window.innerWidth<430?T.hi:T.greeting(new Date().getHours()))}, {name}</p>
...
const LocView = ({user,locId,uid,progress,onBack,onComplete,profile,onLessonComplete,onActivity}) => {
  ...
  const [freeAns,setFreeAns]=useState("");const [feedback,setFeedback]=useState("");const [grading,setGrading]=useState(false);const [showHint,setShowHint]=useState(false);const [freeMin,setFreeMin]=useState(null);
  ...
  const echoesPrompt=(ans)=>{
    ...
    const a=norm(ans);if(a.length<6)return "short";
    ...
    return (overlap>.7||fresh<4)?"similar":"";
  };
  const gradeFreeResponse=async()=>{
    const why=currentP.own?echoesPrompt(freeAns):"";
    if(why){setEchoWarn(why==="short"?T.tooShort:T.tooSimilar);SFX.play("wrong");return}
    setEchoWarn(false);
    setGrading(true);analytics.track("practice_submit",{path:locId,lesson:lessonIdx,q:practiceIdx});
    try{const r=await db.callClaude({feature:"tutor",system:`You are Lumi, grading a practice exercise in an AI-skills course. The question was: "${currentP.q}". Guiding questions the learner saw: "${currentP.hint||""}". A worked example from the lesson (the learner should NOT copy it): "${currentP.example||""}". IMPORTANT: Start your response with exactly "SCORE: X/10" on the first line (where X is 1-10). Scoring rules: 8-10 = the learner's OWN real situation with concrete, specific details (names, numbers, constraints, audience) AND the lesson's framework applied; 5-7 = follows the framework but is generic or thin on personal detail; 1-4 = mostly restates the question, the guiding questions, or the example, or contains no specifics of their own. Then give brief, warm feedback: what's strong, and ONE specific way to make it more concrete or more theirs. Under 120 words. Never condescending.${getLang()!=="en"?` Respond ENTIRELY in ${LANGS[getLang()].name} (except the SCORE: X/10 line which must stay in English).`:""}`,messages:[{role:"user",content:freeAns}]});
      ...
      // The answer already passed the echo guard, so it is a sincere attempt:
      // floor at 3 (the grader will happily return 0 for a thin answer).
      const numScore=Math.min(10,Math.max(3,scoreMatch?parseInt(scoreMatch[1]):6));
      setFreeMin(m=>m===null?numScore:Math.min(m,numScore));
      ...
      setGrading(false);setFeedback("");
      if(e?.code==="auth_required"||e?.code==="quota"){setEchoWarn(false);return}
      console.warn("grading failed:",e);analytics.track("practice_error",{reason:e?.code||"error"});setEchoWarn(T.gradeFailed);return}
  ...
  const finishLesson=(idx,pct)=>{
    analytics.track("lesson_complete",{path:locId,lesson:idx,pct});
  ...
  const resetPractice=()=>{setPracticeIdx(0);setSelected(null);setSubmitted(false);setFreeAns("");setFeedback("");setPracticeScore(0);setTotalPossible(0);setShowConfetti(false);setShowResults(false);setShowHint(false);setEchoWarn(false);setFreeMin(null)};
  const goToLesson=(idx)=>{analytics.track("lesson_open",{path:locId,lesson:idx});setLessonIdx(idx);setView("lesson");setMsgs([]);setSid(null);resetPractice()};
  ...
  if(view==="practice"&&practice.length>0&&!showResults)return(<div style={{height:"100vh",overflowY:"auto",background:`linear-gradient(180deg,${C.bgDark},${C.bgCard})`,padding:"14px 20px 100px"}}>
    ...
    {currentP.type==="free_response"&&<div>
      ...
      {echoWarn&&<div className="fu" style={{background:"rgba(216,88,88,.08)",border:`1px solid ${C.red}55`,borderRadius:12,padding:10,marginBottom:10}}><p style={{color:C.red,fontSize:12,lineHeight:1.5,fontFamily:C.font,margin:0}}>{echoWarn===true?T.tooSimilar:echoWarn}</p></div>}
    ...
    {submitted&&<div style={{marginTop:14}}>
      ...
      :<Btn v="gold" onClick={()=>{const p=totalPossible>0?Math.round((practiceScore/totalPossible)*100):0;const ok=p>=70||(freeMin!==null&&freeMin>=7);try{SFX.play(ok?"triumph":p>=50?"sparkle":"fail")}catch(e){}setShowResults(true)}}>{T.seeResults}</Btn>}
    </div>}
  </div>);

  // RESULTS SCREEN
  if(view==="practice"&&showResults){
    // Pass at 70 %, OR when the learner's own-situation exercise scored ≥7 —
    // that carries them to Ridge. With 2 MC + 1 free-response, one wrong MC used
    // to require 11/10 on the essay: an impossible, forced retry.
    const rawPct=totalPossible>0?Math.round((practiceScore/totalPossible)*100):0;
    const ownCarried=rawPct<70&&freeMin!==null&&freeMin>=7;
    const pct=ownCarried?70:rawPct;
    ...
    return(<div style={{height:"100vh",overflowY:"auto",background:`linear-gradient(180deg,${C.bgDark},${C.bgCard})`,padding:"20px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
      ...
      <div className="fu" style={{textAlign:"center",maxWidth:340}}>
        ...
        {ownCarried&&<p className="fu" style={{color:C.teal,fontSize:12,lineHeight:1.5,fontFamily:C.font,margin:"0 0 10px"}}>✍️ {T.ownCarried}</p>}
        ...
        {passed?<div style={{display:"flex",flexDirection:"column",gap:8}}>
          ...
          {pct<90&&<Btn v="ghost" onClick={()=>{setShowResults(false);setPracticeIdx(0);setSelected(null);setSubmitted(false);setFreeAns("");setFeedback("");setPracticeScore(0);setTotalPossible(0);setFreeMin(null);setView("practice")}}>{T.retry}</Btn>}
        ...
        :<div style={{display:"flex",flexDirection:"column",gap:8}}>
          ...
          <Btn v="gold" onClick={()=>{setShowResults(false);setPracticeIdx(0);setSelected(null);setSubmitted(false);setFreeAns("");setFeedback("");setPracticeScore(0);setTotalPossible(0);setFreeMin(null);setView("practice")}}>{T.tryAgain}</Btn>
  ...
  if(view==="lesson"&&lesson)return(<div style={{height:"100vh",overflowY:"auto",background:`linear-gradient(180deg,${C.bgDark},${C.bgCard})`,padding:"14px 20px 100px"}}>
    ...
    {practice.length>0?<Btn v="teal" onClick={()=>{analytics.track("practice_start",{path:locId,lesson:lessonIdx});setView("practice");resetPractice()}}>{T.startPractice}</Btn>
...
// "Today" was being stamped on week-old stories. Show the real publication
// date instead (falls back to whatever the old cache had).
const newsWhen=(n)=>{
  if(n?.published){const d=new Date(n.published+"T12:00:00");if(!isNaN(d)){const days=Math.round((Date.now()-d.getTime())/86400000);if(days<=0)return T.today;if(days===1)return T.yesterday;return d.toLocaleDateString(getLang()==="ar"?"ar":getLang()==="fr"?"fr":"en-US",{month:"short",day:"numeric"})}}
  return n?.timeAgo||"";
};
const openExternal=(url)=>{try{if(_isNative&&window.Capacitor?.Plugins?.Browser){window.Capacitor.Plugins.Browser.open({url});return}}catch{}try{window.open(url,"_blank","noopener")}catch{}};
const NewsView = ({uid,onBack,onActivity}) => {
  ...
  const fetchNews=async(force=false)=>{
    ...
    try{
      ...
      const r=await db.callClaude({feature:"news_fetch",use_search:true,lang:getLang(),force,system:`You are Lumi, an AI news curator. Today is ${today}. Search for the most important AI news from the last 3 days (prefer today). Return EXACTLY 4 items as a JSON array. Each item: title (string), category (Breaking/Tools/Policy/Business/Research), summary (2-3 sentence ELI5), impact (why it matters to an average person), published (the story's real publication date as YYYY-MM-DD — never guess "today"), source (the PRIMARY outlet that reported it, e.g. "Reuters", "TechCrunch", "The Verge", "NVIDIA Blog", "OpenAI" — not aggregators, newsletters or content farms), url (the direct https link to that outlet's article), timeAgo (leave empty). Prefer major outlets and official company blogs; skip a story if you cannot cite a reputable primary source. Return ONLY a valid JSON array, no markdown, no backticks, no extra text.${langNote}`,messages:[{role:"user",content:`Find the 4 most important AI news stories as of ${today}, each with its primary source and link.`}]});
      try{
  ...
  if(art)return(<div style={{height:"100vh",overflowY:"auto",background:C.bgDark,padding:"14px 20px 100px"}}>
    ...
    <div className="fu" style={{display:"flex",alignItems:"center",gap:6,marginBottom:8,flexWrap:"wrap"}}><span style={{fontSize:11,color:C.gold,fontWeight:700,fontFamily:C.font,textTransform:"uppercase",letterSpacing:.8}}>{art.category}</span>{newsWhen(art)&&<span style={{color:C.textDim,fontSize:11,fontFamily:C.font}}>{newsWhen(art)}</span>}{art.source&&<span style={{color:C.textDim,fontSize:11,fontFamily:C.font}}>· {art.source}</span>}</div>
    ...
    {art.url&&<button onClick={()=>{analytics.track("news_source_open",{source:art.source});openExternal(art.url)}} className="fu s4" style={{width:"100%",background:"transparent",border:`1px solid ${C.border}`,borderRadius:14,padding:"11px 14px",textAlign:"left",marginBottom:10,display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{color:C.text,fontSize:13,fontWeight:700,fontFamily:C.font}}>{T.readOriginal}</span><span style={{color:C.textDim,fontSize:11,fontFamily:C.font}}>{art.source}</span></button>}
  ...
  return(<div style={{height:"100vh",overflowY:"auto",background:C.bgDark,padding:"14px 20px 40px"}}>
    ...
    :<div style={{display:"flex",flexDirection:"column",gap:10}}>{articles.map((n,i)=>(<button key={i} className={`fu s${Math.min(i+1,5)}`} onClick={()=>setOpen(i)} style={{background:C.bgCard,border:`1px solid ${C.border}`,borderRadius:14,padding:14,textAlign:"left",width:"100%"}}>
      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:4}}><span style={{fontSize:11,color:C.gold,fontWeight:700,fontFamily:C.font,textTransform:"uppercase",letterSpacing:.8}}>{n.category}</span><span style={{color:C.textDim,fontSize:11,fontFamily:C.font}}>{newsWhen(n)}</span>{n.source&&<span style={{color:C.textDim,fontSize:11,fontFamily:C.font}}>· {n.source}</span>}</div>
      <p style={{color:C.text,fontSize:14,fontWeight:600,fontFamily:C.font,margin:0,lineHeight:1.4}}>{n.title}</p>
    </button>))}</div>}
  </div>);
};

// CHAT WITH LUMI — an open sandbox. Beginners need a place to just *talk to an
// AI* inside the app (the lesson chat is scoped to one lesson, and the Daily
// Challenge used to send people off to ChatGPT).
const LumiChatView = ({uid,profile,user,progress,seed,onBack,onActivity}) => {
  const [msgs,setMsgs]=useState([]);const [typing,setTyping]=useState(false);const [inp,setInp]=useState(seed||"");const [sid,setSid]=useState(null);const ref=useRef(null);
  useEffect(()=>{ref.current?.scrollTo(0,ref.current.scrollHeight)},[msgs,typing]);
  const persona=getPersona(profile,user);
  const userName=(profile?.display_name||user?.email?.split("@")[0]||"there");
  const langInstruction=getLang()!=="en"?`Respond ENTIRELY in ${LANGS[getLang()].name}.`:"";
  const sys=`You are Lumi, the warm, practical AI guide inside "Lumicamp", an app that teaches beginners to use AI. This is the open chat: the learner can ask anything — help writing something, explaining a term, planning, brainstorming, checking an idea. Do the task for them properly (write the email, make the list) AND, briefly, point out what made their request work or how to make it clearer next time — one sentence, only when useful. The learner's name is ${userName}. Their AI experience: "${persona.exp||"unknown"}" — ${isExperienced(persona)?"skip the basics":"assume nothing, define terms the first time"}. Goals: ${persona.goals.join(", ")||"not stated"}. Plain language, everyday analogies, never condescending. Never ask them to paste passwords, ID numbers or other sensitive data; if they share some, gently say so. Under 220 words unless they ask for something longer. ${langInstruction}`;
  const ask=async(q)=>{
    if(uid&&!msgs.some(m=>m.from==="user")){db.bumpTutorSessions(uid).then(()=>onActivity?.()).catch(()=>{});Streak.recordActivity();db.recordActivity(uid,"chat").then(({data})=>Streak.applyServer(data))}
    analytics.track("chat_send",{where:"sandbox",n:msgs.filter(m=>m.from==="user").length+1});
    setMsgs(m=>[...m,{from:"user",text:q}]);setTyping(true);
    try{const r=await db.callClaude({feature:"tutor",system:sys,messages:[...msgs.map(m=>({role:m.from==="user"?"user":"assistant",content:m.text})),{role:"user",content:q}],session_id:sid,context_type:"sandbox",context_id:"chat",context_title:"Chat with Lumi"});
      setSid(r.session_id||sid);setTyping(false);setMsgs(m=>[...m,{from:"lumi",text:r.text}]);
    }catch(e){setTyping(false);setMsgs(m=>[...m,{from:"lumi",text:(e?.code==="auth_required"||e?.code==="quota")?e.message:"Hmm, I lost my connection for a moment. Could you try that again? 🌟"}])}};
  const send=()=>{if(inp.trim()&&!typing){ask(inp.trim());setInp("")}};
  useBackHandler(()=>{onBack();return true});
  const level=Math.max(1,Math.floor((progress||[]).length/2)+1);
  return(<div style={{height:"100vh",display:"flex",flexDirection:"column",background:C.bgDark}}>
    <div style={{padding:"12px 16px",paddingTop:(TOP_SAFE+12)+"px",borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:8,flexShrink:0,background:C.bgCard}}>
      <button onClick={onBack} style={{background:"none",border:"none",color:C.gold,fontSize:14,fontFamily:C.font,fontWeight:700}}>{T.map}</button>
      <div style={{flex:1,textAlign:"center",display:"flex",alignItems:"center",justifyContent:"center",gap:6}}><Lumi size={22} level={level}/><span style={{color:C.text,fontSize:13,fontWeight:700,fontFamily:C.font}}>{T.chatWithLumi}</span></div>
      <span style={{width:52}}/>
    </div>
    <div ref={ref} style={{flex:1,overflowY:"auto",padding:16}}>
      <Bub from="lumi" text={`${T.hi} ${userName}! ${T.chatIntro}`}/>
      {msgs.map((m,i)=><Bub key={i} from={m.from} text={m.text} copyable/>)}{typing&&<Bub from="lumi" typing/>}
      {msgs.length===0&&!seed&&<div className="fu s2" style={{marginTop:14}}><p style={{color:C.textDim,fontSize:12,fontFamily:C.font,fontWeight:600,margin:"0 0 10px"}}>{T.peoplAsk}</p>{(T.chatStarters||[]).map((q,i)=><button key={i} onClick={()=>ask(q)} style={{display:"block",width:"100%",background:"rgba(255,255,255,.03)",border:`1px solid ${C.border}`,borderRadius:12,padding:"10px 14px",marginBottom:7,textAlign:"left"}}><span style={{color:C.textMuted,fontSize:13,fontFamily:C.font}}>{q}</span></button>)}</div>}
    </div>
    <div style={{padding:"8px 14px",paddingBottom:(12+BOTTOM_SAFE)+"px",borderTop:`1px solid ${C.border}`,display:"flex",gap:8,flexShrink:0,background:C.bgCard}}>
      <textarea value={inp} onChange={e=>setInp(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();send()}}} placeholder={T.askLumi+"..."} rows={Math.min(5,Math.max(1,(inp.match(/\n/g)||[]).length+1))} style={{flex:1,background:"rgba(255,255,255,.04)",borderRadius:12,border:`1px solid ${C.border}`,padding:"11px 14px",color:C.text,fontSize:14,fontFamily:C.font,outline:"none",resize:"none"}}/>
      <button onClick={send} aria-label="Send" disabled={typing} style={{width:42,height:42,borderRadius:12,background:`linear-gradient(135deg,${C.gold},${C.goldDark})`,border:"none",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,alignSelf:"flex-end",opacity:typing?.6:1}}><span style={{color:"#fff",fontSize:16}}>↑</span></button>
    </div></div>);
};

// DAILY CHALLENGE
const ChallengeView = ({uid,profile,user,onBack,onActivity,onOpenChat}) => {
  const today=new Date();
  ...
  const [response,setResponse]=useState(saved?.response||"");const [feedback,setFeedback]=useState(saved?.feedback||"");const [submitted,setSubmitted]=useState(!!saved);const [grading,setGrading]=useState(false);const [showConfetti,setShowConfetti]=useState(false);const [challengeErr,setChallengeErr]=useState("");
  const [history,setHistory]=useState([]);
  useEffect(()=>{if(uid)db.getChallengeLog(uid).then(setHistory).catch(()=>{})},[uid,submitted]);

  const submit=async()=>{
    setGrading(true);setChallengeErr("");analytics.track("challenge_submit");
    let scoreNum=null;
    try{const r=await db.callClaude({feature:"tutor",system:`You are Lumi, grading a daily challenge. The challenge was: "${challenge.title}" - "${challenge.task}". Start with exactly "SCORE: X/10" on the first line. Reserve 8+ for answers with the learner's OWN concrete details (real situation, names, numbers, constraints); 4 or below if it mostly restates the task or is generic. Then: what they did well, and ONE specific improvement suggestion. Be encouraging, never condescending. Under 150 words.${getLang()!=="en"?` Respond ENTIRELY in ${LANGS[getLang()].name} (keep the SCORE line in English).`:""}`,messages:[{role:"user",content:response}],context_type:"tool",context_id:challenge.id,context_title:"Daily Challenge: "+challenge.title});
      ...
      setGrading(false);
      if(e?.code!=="auth_required"&&e?.code!=="quota"){console.warn("challenge grading failed:",e);setChallengeErr(T.gradeFailed)}
      return}
    setGrading(false);setSubmitted(true);
  ...
  return(<div style={{height:"100vh",overflowY:"auto",background:`linear-gradient(180deg,${C.bgDark},${C.bgCard})`,padding:"14px 20px 40px"}}>
    ...
    {!submitted?<>
      ...
      {onOpenChat&&<button onClick={()=>{analytics.track("challenge_try_chat");onOpenChat(challenge.task)}} className="fu" style={{width:"100%",background:"rgba(58,168,160,.08)",border:"1px solid rgba(58,168,160,.25)",borderRadius:14,padding:"12px 14px",textAlign:"left",marginBottom:12,display:"flex",alignItems:"center",gap:10}}><Lumi size={28}/><div><p style={{color:C.teal,fontSize:14,fontWeight:700,fontFamily:C.font,margin:0}}>{T.tryWithLumi}</p><p style={{color:C.textDim,fontSize:11,fontFamily:C.font,margin:"2px 0 0"}}>{T.tryWithLumiDesc}</p></div></button>}
      {challengeErr&&<div className="fu" style={{background:"rgba(216,88,88,.08)",border:`1px solid ${C.red}55`,borderRadius:12,padding:10,marginBottom:10}}><p style={{color:C.red,fontSize:12,lineHeight:1.5,fontFamily:C.font,margin:0}}>{challengeErr}</p></div>}
...
const getReminder=()=>{try{return JSON.parse(localStorage.getItem(REMIND_KEY)||"null")||{on:false,time:"19:00"}}catch{return{on:false,time:"19:00"}}};
const Reminder={
  // Wrapped in an object: a Capacitor plugin proxy must never be returned
  // directly from an async function (JS probes `.then` on it → "not implemented on web").
  async plugin(){try{const m=await import("@capacitor/local-notifications");return{LN:m.LocalNotifications}}catch{return{LN:null}}},
  async apply(prefs){
    try{localStorage.setItem(REMIND_KEY,JSON.stringify(prefs))}catch{}
    const {LN}=await Reminder.plugin();if(!LN)return{ok:false,reason:"unsupported"};
    try{
      await LN.cancel({notifications:[{id:1}]}).catch(()=>{});
      if(!prefs.on)return{ok:true};
      ...
      const [h,m]=String(prefs.time||"19:00").split(":").map(Number);
      if(_isNative){await LN.schedule({notifications:[{id:1,title:T.reminderTitle,body:T.reminderBody,schedule:{on:{hour:h,minute:m},allowWhileIdle:true},smallIcon:"ic_stat_icon_config_sample"}]});return{ok:true}}
      // Web: the plugin can only fire while the page is open — schedule the next occurrence.
      const at=new Date();at.setHours(h,m,0,0);if(at<=new Date())at.setDate(at.getDate()+1);
      await LN.schedule({notifications:[{id:1,title:T.reminderTitle,body:T.reminderBody,schedule:{at}}]});return{ok:true,web:true};
    }catch(e){console.warn("reminder:",e);return{ok:false,reason:"error"}}
  },
};
...
const ProfileView = ({user,profile,progress,onBack,onSignOut,onToggleTheme,onChangeLang,onSignIn,onAccountGone,onOpenTeam}) => {
  ...
  const [rem,setRem]=useState(()=>getReminder());const [remNote,setRemNote]=useState("");
  const [anOff,setAnOff]=useState(()=>analyticsOff());
  const updateReminder=async(next)=>{setRem(next);const r=await Reminder.apply(next);setRemNote(!r.ok?(r.reason==="denied"?T.reminderDenied:T.reminderWeb):(r.web?T.reminderWeb:""));analytics.track("reminder_set",{on:next.on,time:next.time})};
  ...
  return(<div style={{height:"100vh",overflowY:"auto",background:`linear-gradient(180deg,${C.skyTop},${C.bgDark})`,padding:"14px 20px 40px",position:"relative"}}><Stars/>
    ...
    {/* Daily reminder */}
    <div className="fu s3" style={{background:C.bgCard,border:`1px solid ${C.border}`,borderRadius:16,padding:16,marginBottom:16,position:"relative",zIndex:1}}>
      <p style={{color:C.text,fontSize:14,fontWeight:700,fontFamily:C.font,margin:"0 0 2px"}}>🔔 {T.reminders}</p>
      <p style={{color:C.textDim,fontSize:12,fontFamily:C.font,margin:"0 0 10px"}}>{T.remindersDesc}</p>
      <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
        <button role="switch" aria-checked={rem.on} onClick={()=>updateReminder({...rem,on:!rem.on})} style={{background:rem.on?"rgba(74,186,120,.15)":"transparent",border:`1px solid ${rem.on?C.green:C.border}`,borderRadius:10,padding:"8px 12px",color:rem.on?C.green:C.textMuted,fontSize:13,fontWeight:700,fontFamily:C.font}}>{rem.on?"✓ ":""}{T.reminderOn}</button>
        <input type="time" value={rem.time} onChange={e=>updateReminder({...rem,time:e.target.value,on:true})} aria-label={T.reminderOn} style={{background:"rgba(255,255,255,.04)",border:`1px solid ${C.border}`,borderRadius:10,padding:"7px 10px",color:C.text,fontSize:13,fontFamily:C.font}}/>
      </div>
      {remNote&&<p style={{color:C.textDim,fontSize:11,fontFamily:C.font,margin:"8px 0 0",lineHeight:1.5}}>{remNote}</p>}
    </div>

    {/* Anonymous usage stats */}
    <div className="fu s3" style={{background:C.bgCard,border:`1px solid ${C.border}`,borderRadius:16,padding:16,marginBottom:16,position:"relative",zIndex:1}}>
      <button role="switch" aria-checked={!anOff} onClick={()=>{const off=!anOff;setAnalyticsOff(off);setAnOff(off)}} style={{width:"100%",background:"transparent",border:"none",padding:0,textAlign:"left",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
        <div><p style={{color:C.text,fontSize:14,fontWeight:700,fontFamily:C.font,margin:"0 0 2px"}}>📊 {T.analyticsToggle}</p><p style={{color:C.textDim,fontSize:12,fontFamily:C.font,margin:0}}>{T.analyticsDesc}</p></div>
        <span style={{flexShrink:0,width:40,height:22,borderRadius:11,background:anOff?"rgba(255,255,255,.1)":C.green,position:"relative",transition:"background .2s"}}><span style={{position:"absolute",top:2,left:anOff?2:20,width:18,height:18,borderRadius:"50%",background:"#fff",transition:"left .2s"}}/></span>
      </button>
    </div>
...
export default function Lumicamp(){
  ...
  useEffect(()=>{
    ...
    analytics.track("app_open",{native:_isNative});
    const init=async()=>{
    ...
    const{data}=db.onAuth((ev,s)=>{
      ...
      if(ev==="SIGNED_IN"&&s?.user){
        const u=s.user;setUser(u);analytics.setUser(u.id);
        const first=loadedUidRef.current!==u.id;loadedUidRef.current=u.id;
        if(first)analytics.track("signin_success");
        setTimeout(async()=>{
      ...
      }else if(ev==="SIGNED_OUT"){loadedUidRef.current=null;analytics.setUser(null);setUser(null);setProfile(null);setActiveOrgId(null);setProgress(getLocalProgress())}
    });
  ...
  if(screen==="location"&&activeLoc==="master")content=<SummitView user={user} profile={profile} progress={progress} onBack={goMap} onOpenChallenge={()=>setScreen("challenge")} onOpenTools={()=>setScreen("tools")} onOpenNews={()=>setScreen("news")} onOpenLoc={(id)=>{setActiveLoc(id);setScreen("location")}}/>;
  ...
  else if(screen==="challenge")content=<ChallengeView uid={user?.id} profile={profile} user={user} onBack={goMap} onActivity={refresh} onOpenChat={(seed)=>{setChatSeed(seed||"");setScreen("chat")}}/>;
  else if(screen==="chat")content=<LumiChatView uid={user?.id} profile={profile} user={user} progress={progress} seed={chatSeed} onBack={()=>{setChatSeed("");goMap()}} onActivity={refresh}/>;
    ...
    onOpenLoc={(id)=>{analytics.track("loc_open",{path:id});setActiveLoc(id);setScreen("location")}}
    onOpenNews={()=>{analytics.track("news_open");setScreen("news")}}
    onOpenTools={()=>{analytics.track("tools_open");setScreen("tools")}}
    onOpenChallenge={()=>{analytics.track("challenge_open");setScreen("challenge")}}
    onOpenChat={()=>{analytics.track("chat_open",{from:"map"});setChatSeed("");setScreen("chat")}}
