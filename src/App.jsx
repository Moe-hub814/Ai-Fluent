...
const LocView = ({user,locId,uid,progress,onBack,onComplete,profile,onLessonComplete}) => {
  ...
  if(view==="practice"&&practice.length>0&&!showResults)return(<div style={{height:"100vh",overflowY:"auto",background:`linear-gradient(180deg,${C.bgDark},${C.bgCard})`,padding:"14px 20px 100px"}}>
    ...
    {currentP.type==="free_response"&&<div>
      {currentP.hint&&!submitted&&<div className="fu" style={{background:"rgba(212,165,90,.04)",border:`1px solid ${C.borderGold}`,borderRadius:12,padding:10,marginBottom:12}}><p style={{color:C.gold,fontSize:12,fontFamily:C.font,margin:0}}>💡 {T.hint}: {currentP.hint}</p></div>}
      <textarea value={freeAns} onChange={e=>setFreeAns(e.target.value)} placeholder={T.typeAnswer} disabled={submitted} style={{width:"100%",minHeight:120,background:"rgba(255,255,255,.03)",borderRadius:14,border:`1px solid ${C.border}`,padding:14,color:C.text,fontSize:14,fontFamily:C.font,outline:"none",resize:"vertical",marginBottom:10}}/>
  ...
  if(view==="lesson"&&lesson)return(<div style={{height:"100vh",overflowY:"auto",background:`linear-gradient(180deg,${C.bgDark},${C.bgCard})`,padding:"14px 20px 100px"}}>
    ...
    <div className="fu" style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}><span style={{fontSize:18}}>{loc.icon}</span><span style={{color:loc.color,fontSize:10,fontWeight:700,fontFamily:C.font,textTransform:"uppercase",letterSpacing:1}}>{loc.sub} · {T.lesson} {lessonIdx+1}</span></div>
    ...
    const fetchNews=async()=>{
      try{
        ...
        try{
          ...
          setArticles([{title:"AI News Loading Issue",category:"Info",summary:"Lumi had trouble formatting today's news. Try refreshing!",impact:"This is a temporary issue.",timeAgo:"now",source:"Lumicamp"}]);
        }
      }catch{setArticles([{title:"Couldn't fetch news right now",category:"Info",summary:"Lumi's news connection is down. Check back soon!",impact:"This is temporary.",timeAgo:"now",source:"Lumicamp"}])}
      setLoading(false);
    };
    fetchNews();
  },[]);
...
const ToolsView = ({uid,onBack}) => {
  ...
  const doGen=async(all)=>{setGen(true);try{const prompt=wf.steps.map((s,i)=>`${s.q}: ${all[i]}`).join("\n");const r=await db.callClaude({feature:"tool",system:wf.sys+" You are Lumi from Lumicamp. Practical, beginner-friendly.",messages:[{role:"user",content:prompt}],context_type:"tool",context_id:wf.id,context_title:wf.name});setResult(r.text)}catch{setResult("Something went wrong — try again.")}setGen(false)};
